               ...
               const struct adaptived_cause * const cse)
{
	...
	cse_opts = (struct adaptivemmd_opts *)xcse->data;
...
int adaptivemmd_effects_main(struct adaptived_effect * const eff)
{
	...
	if (opts->max_triggers && (opts->trigger_count >= opts->max_triggers)) {
		adaptived_info("adaptivemmd_effects_main: opts->max_triggers %d Hit. Bailout.\n",
		    opts->max_triggers);
		return -1;
	}
