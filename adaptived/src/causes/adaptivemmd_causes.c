...
int get_max_numa_nodes()
{
	...
	close(fd);
fprintf(stderr, "XXX bytes read: %d\n", br);
	if (br <= 0) {
		adaptived_err("get_max_numa_nodes: read /sys/devices/system/node/online failed.\n");
		return -errno;
	}
fprintf(stderr, "XXX strlen(buffer) = %ld\n", strlen(buffer));
fprintf(stderr, "XXX buffer: %s", buffer);

	subp = strstr(buffer, "-");
fprintf(stderr, "XXX buffer: %p, subp: %p\n", buffer, subp);
	if (subp) {
		max_numa = strtol(++subp, 0, 0);
	}
	max_numa++;
fprintf(stderr, "XXX max_numa: %d\n", max_numa);
