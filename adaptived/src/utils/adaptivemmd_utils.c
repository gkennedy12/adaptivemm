...
int adaptivemm_open(const char *pathname, int flags)
{
	...
	if (strcmp(pathname, PROC_KPAGECOUNT) == 0) {
		sprintf(new_path, "%s/kpagecount.%d", sample_path, iteration);
	} else if (strcmp(pathname, PROC_KPAGEFLAGS) == 0) {
	...
	if (sample_data_debug)
		adaptived_dbg("adaptivemm_open: %s\n", new_path);
...
FILE *adaptivemm_fopen(const char *pathname, const char *mode)
{
	...
	if (strcmp(pathname, PROC_BUDDYINFO) == 0) {
		sprintf(new_path, "%s/buddyinfo.%d", sample_path, iteration);
	} else if (strcmp(pathname, PROC_ZONEINFO) == 0) {
		sprintf(new_path, "%s/zoneinfo.%d", sample_path, iteration);
	} else if (strcmp(pathname, PROC_VMSTAT) == 0) {
		sprintf(new_path, "%s/vmstat.%d", sample_path, iteration);
	} else if (strcmp(pathname, PROC_MEMINFO) == 0) {
		sprintf(new_path, "%s/meminfo.%d", sample_path, iteration);
	} else if (strcmp(pathname, PROC_MODULES) == 0) {
		sprintf(new_path, "%s/modules.%d", sample_path, iteration);
	} else {
		return NULL;
	}
	if (access(new_path, F_OK) != 0) {
		adaptived_dbg("adaptivemm_fopen: can't find %s\n", new_path);
		return NULL;
	}

	if (sample_data_debug)
		adaptived_dbg("adaptivemm_fopen: %s\n", new_path);
...
DIR *adaptivemm_opendir(const char *pathname, char *dirname)
{
	...
	if (strcmp(pathname, MM_HUGEPAGESINFO) == 0)
		sprintf(new_path, "%s/hugepages.%d", sample_path, iteration);
	else 
		return NULL;
	if (sample_data_debug)
		adaptived_dbg("adaptivemm_opendir: %s\n", new_path);
	memcpy(dirname, new_path, strlen(new_path));
	if (access(new_path, F_OK) != 0) {
		adaptived_dbg("adaptivemm_opendir: can't find %s\n", new_path);
		return NULL;
	}
...
update_hugepages(struct adaptivemmd_opts * const opts)
{
	...
	dp = adaptivemm_opendir(MM_HUGEPAGESINFO, dirname);
	if (dp == NULL) {
	...
	while (((ep = readdir(dp)) != NULL) && (ep->d_type == DT_DIR)) {
		...
		snprintf(tmpstr, sizeof(tmpstr), "%s/%s/nr_hugepages",
			(strlen(dirname)) ? dirname : MM_HUGEPAGESINFO, ep->d_name);
		fp = fopen(tmpstr, "r");
		if (fp == NULL)
			continue;
...
update_zone_watermarks(struct adaptivemmd_opts * const opts)
{
	...
	fp = adaptivemm_fopen(PROC_ZONEINFO, "r");
	if (!fp)
		return -ENOENT;
...
rescale_maxwsf(struct adaptivemmd_opts * const opts)
{
	...
	for (i = 0; i < opts->max_numa_nodes; i++)
...
no_pages_reclaimed(struct adaptivemmd_opts * const opts)
{
	...
	fp = adaptivemm_fopen(PROC_VMSTAT, "r");
	if (!fp)
		return -ENOENT;
...
get_unmapped_pages(struct adaptivemmd_opts * const opts)
{
	...
	if ((fd1 = adaptivemm_open(PROC_KPAGECOUNT, O_RDONLY)) == -1) {
		log_err("Error opening kpagecount\n");
		return -1;
	}
	if ((fd2 = adaptivemm_open(PROC_KPAGEFLAGS, O_RDONLY)) == -1) {
...
pr_meminfo(int level)
{
        ...
        fp = adaptivemm_fopen(PROC_MEMINFO, "r");
        if (!fp)
                return;
...
check_memory_pressure(struct adaptivemmd_opts * const opts, bool init)
{
	...
	if (init) {
		...
		if ((opts->ifile = adaptivemm_fopen(PROC_BUDDYINFO, "r")) == NULL) {
...
check_memory_pressure_effect(struct adaptivemmd_opts * const opts)
{
	...
	if (opts->mem_pressure_trigger != MEMORY_PRESSURE_TRIGGER) {
		log_err("%s: trigger type should be %d, but got %d\n",
		    __func__, MEMORY_PRESSURE_TRIGGER, opts->mem_pressure_trigger);
		return -EINVAL;
	}

	for (nid = 0; nid < opts->max_numa_nodes; nid++) {
	...
	if (result & (MEMPREDICT_RECLAIM | MEMPREDICT_LOWER_WMARKS)) {
		if (result & MEMPREDICT_RECLAIM) {
			adaptived_dbg("check_memory_pressure_effect: watermarks scaled up.\n");
		} else {
			adaptived_dbg("check_memory_pressure_effect: watermarks scaled down.\n");
		}
	}
	return 0;
}
...
int run_adaptivemm_init(struct adaptivemmd_opts * const opts, int interval)
{
	...
	case HIGH_PERIODICITY:          /* 15 second interval */
		...
		break;
	default:
		if (opts->memleak_check_enabled || opts->capture_data_dir) {
			/* if memleak_check_enabled or replay, allow */
			opts->aggressiveness = 3;
			opts->maxwsf = 1000;
			opts->max_compaction_order = MAX_ORDER - 2;
			opts->periodicity = HIGH_PERIODICITY;
	...
	pr_info("adaptivemmd %s started (verbose=%d, debug_mode=%d, aggressiveness=%d, maxgap=%d)\n",
...
int run_adaptivemm_effects(struct adaptivemmd_opts * const opts)
{
	...
	if (!trigger_handled) {
		log_err("%s: effects called, but no handled.\n", __func__);
		ret = -EINVAL;
		goto error;
	}
	return ret;
