...
int main(int argc, char *argv[])
{
	...
	sprintf(cmdline, "tar xf %s/%s/mem_leak_logs.tar.gz --directory %s/%s/.",
		argv[1], sample_dir, argv[1], sample_dir);
	...
	if (ret != EXPECTED_RET)
		goto err;
	sprintf(cmdline, "%s/%s/mem_leak_logs/adaptivemm.log", argv[1], sample_dir);
