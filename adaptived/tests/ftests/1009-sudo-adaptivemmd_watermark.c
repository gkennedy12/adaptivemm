...
int validate_replay_log(char *expected_file, char *replay_file)
{
		...
		if (strncmp(line0, "Updating negative dentry limit ", strlen("Updating negative dentry limit ")) == 0) {
			/* Skip check if NEG_DENTRY_LIMIT does not exist */
			if (access(NEG_DENTRY_LIMIT, F_OK) != 0)
				continue;
		}
		if (strncmp(line0, "** reclamation rate is ", strlen("** reclamation rate is ")) == 0) {
			adaptived_dbg("sudo1009: skip reclamation line0: %s", line0);
			continue;
		}
next1:
		...
		if (strncmp(line1, "** reclamation rate is ", strlen("** reclamation rate is ")) == 0) {
			adaptived_dbg("sudo1009: skip reclamation line1: %s", line1);
			goto next1;
		}
