...
int main(int argc, char *argv[])
{
	...
	sprintf(cmdline, "%s/%s/watermark_logs", argv[1], sample_dir);
	adaptived_dbg("cmdline: %s\n", cmdline);
	if (access(cmdline, F_OK) == 0) {
		adaptived_dbg("%s exists!!!\n", cmdline);
	} else {
		sprintf(cmdline, "tar xf %s/%s/watermark_logs.tar.gz --directory %s/%s/.", argv[1], sample_dir, argv[1], sample_dir);
		adaptived_dbg("cmdline: %s\n", cmdline);
		ret = system(cmdline);
		if (ret)
			goto err;
	...
	if (ret != EXPECTED_RET)
		goto err;
	sprintf(cmdline, "%s/%s/watermark_logs/adaptivemm.log", argv[1], sample_dir);
	adaptived_dbg("cmdline: %s\n", cmdline);
